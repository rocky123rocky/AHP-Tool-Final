...
def login():
    ...
    <div class="title-section">
        ...
        <h1 style='font-size:2.2rem;margin:8px 0;text-shadow:2px 2px 4px rgba(0,0,0,0.5);'>ANALYTICAL HIERARCHY PROCESS (AHP) FRAMEWORK IN<br>COMMON OPERATIONAL PLANNING PROCESS</h1>
        <h2 style='font-size:1.1rem;margin:4px 0;font-style:italic;'>(Assessment of progress in Decisive Points, Objectives & Phases)</h2>
        <h3 style='font-size:1.2rem;margin:4px 0;'>⚓ NAVAL WING, DSSC Wellington ⚓</h3>
    ...
    if additional_forces:
        ...
        for idx, side in enumerate(additional_forces):
            with add_cols[idx]:
                ...
                if st.button(f"🔶 {side.upper()} FORCE", key=f"big_{side}_btn_login"):
...
def dps_tab():
    ...
    if role in ["control"] + SIDES:
        ...
        if all_objectives:
            ...
            # Auto-generate unique DP number within current force only
            current_force = role if role != "control" else side
            current_force_data = load_project(project, current_force)
            existing_dp_nos_in_force = set()
            for dp in current_force_data.get("dps", []):
                dp_no_val = dp.get("DP No")
                if dp_no_val is not None:
                    existing_dp_nos_in_force.add(int(dp_no_val) if str(dp_no_val).isdigit() else 0)
            
            # Find next available DP number within this force
            suggested_dp_no = 1
            while suggested_dp_no in existing_dp_nos_in_force:
                suggested_dp_no += 1
                
            dp_no = st.number_input("DP Number (Unique within force)", min_value=1, value=suggested_dp_no, key="new_dp_no", help=f"Suggested: {suggested_dp_no} (next unique DP number for {current_force} force)")
            
            # Validate uniqueness within current force
            if dp_no in existing_dp_nos_in_force:
                st.error(f"DP Number {dp_no} already exists in {current_force} force! Please choose a different number.")
                st.stop()
    ...
    if dps:
        ...
        if selected_dp_idx is not None:
            ...
            with col1:
                ...
                # Get existing DP numbers for uniqueness check within current force only
                force_existing_dp_nos = set()
                current_force_data = load_project(project, selected_force_edit)
                for dp in current_force_data.get("dps", []):
                    dp_no_val = dp.get("DP No")
                    if dp_no_val is not None and dp != selected_dp:  # Exclude current DP
                        force_existing_dp_nos.add(int(dp_no_val) if str(dp_no_val).isdigit() else 0)
                
                current_dp_no = selected_dp.get("DP No", 1)
                new_dp_no = st.number_input("DP Number (Unique within force)", min_value=1, value=int(current_dp_no) if str(current_dp_no).isdigit() else 1, key="edit_dp_no")
                
                # Check uniqueness within current force
                dp_no_valid = new_dp_no not in force_existing_dp_nos or new_dp_no == current_dp_no
                if not dp_no_valid:
                    st.error(f"DP Number {new_dp_no} already exists in {selected_force_edit} force! Please choose a different number.")
                ...
                current_dp_weight = selected_dp.get("Weight", 3)
                try:
                    current_dp_weight = float(str(current_dp_weight)) if current_dp_weight else 3.0
                except:
                    current_dp_weight = 3.0
                new_weight = st.slider("Weight/Priority", 1, 5, int(current_dp_weight), key="edit_dp_weight")
    ...
    if tasks:
        ...
        if selected_task_idx is not None:
            ...
            with col1:
                ...
                new_weight = st.slider("Weight/Priority", 0.0, 100.0, float(current_weight), step=0.1, key="edit_task_weight")
                new_progress = st.slider("Progress (%)", 0.0, 100.0, float(current_progress), step=0.1, key="edit_task_progress")
...
def dashboard_tab():
    ...
    for idx, side in enumerate(SIDES):
        ...
        with [col1, col2][idx % 2]:
            ...
            # DP Horizontal Bar Chart for better readability
            ...
            for dp_no in dp_numbers:
                ...
                for dp in dps_data:
                    if str(dp.get("DP No", "") or dp.get("dp_no", "")) == str(dp_no):
                        dp_name = dp.get("Name", f"DP {dp_no}")
                        break
                # Clean, single-line format: "DP 1: Description"
                clean_name = dp_name[:40] + "..." if len(dp_name) > 40 else dp_name
                dp_labels.append(f"DP {dp_no}: {clean_name}")
            
            # Enhanced RAG coloring with better colors and status indicators
            rag_colors = []
            status_indicators = []
            for v in dp_vals:
                if v < rag["red"]:
                    rag_colors.append("#dc2626")  # Bright red for critical
                    status_indicators.append("🔴 RED")
                elif v < rag["amber"]:
                    rag_colors.append("#f59e0b")  # Bright amber for caution
                    status_indicators.append("🟡 AMBER")
                else:
                    rag_colors.append("#16a34a")  # Bright green for good
                    status_indicators.append("🟢 GREEN")
            ...
            fig.update_layout(
                title=f"DP Progress - RAG Status (🔴 Red <{rag['red']}%, 🟡 Amber <{rag['amber']}%, 🟢 Green ≥{rag['amber']}%)", 
                xaxis_title="Progress (%)", 
                yaxis_title="Decisive Points",
                height=max(300, len(dp_labels) * 45),  # Back to original sizing
                margin=dict(l=200, r=50, t=70, b=50),  # Reduced left margin, kept top for title
                yaxis=dict(automargin=True),
                font=dict(size=10)
            )
            st.plotly_chart(fig, use_container_width=True, key=f"dp_chart_{side}")
            # Objective Horizontal Bar Chart
            obj_vals = list(progress["objective"].values())
            obj_names = list(progress["objective"].keys())
            # Colorful bars for objectives with gradient colors
            obj_colors = [
                "#3b82f6",  # Blue
                "#10b981",  # Emerald
                "#f59e0b",  # Amber
            ...
            # Cycle through colors if more objectives than colors available
            ...
            fig2.update_layout(
                ...
                height=max(300, len(obj_names) * 50),  # Dynamic height based on number of objectives
                margin=dict(l=150, r=50, t=50, b=50)  # Left margin for objective names
            )
            st.plotly_chart(fig2, use_container_width=True, key=f"obj_chart_{side}")
            # Phase Horizontal Bar Chart
            ...
            # Cycle through colors if more phases than colors available
            ...
            fig3.update_layout(
                ...
                xaxis_title="Progress (%)", 
                yaxis_title="Phases",
                height=max(250, len(phase_names) * 60),  # Dynamic height based on number of phases
                margin=dict(l=120, r=50, t=50, b=50),  # Left margin for phase names
                yaxis=dict(automargin=True),
                font=dict(size=11)
            )
